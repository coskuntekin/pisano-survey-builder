...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const loadSurveys = () => {
      ...
      setSurveys(storedSurveys.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()));
    };
  ...
  const handleDownloadSurvey = (survey: Survey) => {
    ...
    downloadAnchorNode.setAttribute("download", `survey-${survey.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.json`);
  ...
  const handleDownloadSelected = () => {
    ...
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(selectedSurveyData, null, 2));
    const downloadAnchorNode = document.createElement('a');
    downloadAnchorNode.setAttribute("href", dataStr);
    downloadAnchorNode.setAttribute("download", `selected-surveys-${new Date().toISOString().split('T')[0]}.json`);
    document.body.appendChild(downloadAnchorNode);
    downloadAnchorNode.click();
    downloadAnchorNode.remove();
  };
  ...
  function SortableRow({ survey }: { survey: Survey }) {
    ...
    return (
      <tr ref={setNodeRef} style={style} {...attributes} className={`hover:bg-gray-50 ${isDragging ? "bg-blue-50" : ""}`}>
        ...
        <td className="px-6 py-4">
          <div className="flex flex-col">
            <div className="font-medium text-gray-900">{survey.title || "Untitled Survey"}</div>
            <div className="text-sm text-gray-500 mt-1">{survey.description || "No description"}</div>
          </div>
        </td>
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-gray-600">Manage and organize your surveys</p>
            </div>
            <button
              onClick={() => navigate("/app/survey-builder/step-1")}
          ...
          {filteredSurveys.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="mt-1 text-sm text-gray-500">
                {searchTerm ? "Try adjusting your search terms." : "Get started by creating a new survey."}
              </p>
              {!searchTerm && (
                <div className="mt-6">
                  <button
            ...
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <SortableContext items={filteredSurveys.map(s => s.id)} strategy={verticalListSortingStrategy}>
